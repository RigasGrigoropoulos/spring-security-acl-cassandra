...
public interface CassandraAclRepository {
	...
	/**
	 * Loads the {@link AclEntry} instances that apply for the passed {@link AclObjectIdentity} objects. The returned map is
     * keyed on the passed objects, with the values being sets of the applicable {@link AclEntry} instances, 
     * sorted by the order parameter. Any unknown objects will not have a map key.
     * 
	...
	/**
	 * Loads a fully populated {@link AclObjectIdentity} object from the database for the provided {@link AclObjectIdentity}.
	 * The provided object must contain 'id' and 'objectClass' information.
	 * 
	...
	/**
	 * Saves an {@link AclObjectIdentity} record in the database. It will have no entries.
	 * 
	 * @param aoi the {@link AclObjectIdentity} to save.
	 * @throws AclAlreadyExistsException if the passed object identity already has a record.
	 */
